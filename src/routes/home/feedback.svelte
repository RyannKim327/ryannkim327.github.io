...
<div
  id="feedback"
  class="flex flex-wrap gap-2 p-2 pt-[25%] md:pt-[calc(5%+0.5rem)] h-full w-full items-start justify-center overflow-y-auto snap-start"
>
  {#if feedbacks.length > 0 && parseData}
    {#each feedbacks as feedback}
      <Card
        class="aspect-video w-full md:w-[calc(33.333%-0.5rem)] justify-start gap-2"
      >
        <div class="flex flex-col">
          <span class="font-bold text-[1.5rem]">{feedback.application}</span>
          <span class="text-[0.75rem]"
            >{feedback.userId ?? "No user provided"}</span
          >
        </div>
