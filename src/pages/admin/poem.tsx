...
export default function Poetry() {
  ...
  const submitPoem = async () => {
    ...
    const comp = {
      title: compose.title,
      content: compose.content.trim().replace(/\n\n/gi, "\n").split("\n"),
    };
  ...
  return (
    <div className="flex flex-row gap-2 w-full items-start h-full">
      ...
      <div className="hidden md:flex flex-col gap-2 items-start overflow-hidden h-full w-1/2">
        <span>{compose.title}</span>
        <div className="h-full w-full overflow-y-auto wrap-break-word">
          {compose.content.split("\n").map((content) => {
            return <p>{content}</p>;
          })}
        </div>
      </div>
